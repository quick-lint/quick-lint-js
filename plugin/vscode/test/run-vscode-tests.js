...
function getVSCodePlatform() {
  if (process.platform === "win32") {
    // HACK(strager): On Windows, vscode-test defaults to 32-bit VS Code. Use
    // 64-bit VS Code instead so our extension works on CI (which currently only
    // builds a 64-bit variant).
    // TODO(strager): Accept the VS Code platform via command-line options.
    return "win32-x64-archive";
  } else {
    return null; // Use vscode-test's default.
  }
}
