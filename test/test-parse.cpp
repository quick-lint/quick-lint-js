...
class parser {
 ...
 private:
  ...
  void parse_let_bindings(Visitor &v) {
    ...
    switch (t.type) {
      default:
        v.visit_error_let_with_no_bindings(let.range(this->original_input_));
        break;
...
struct visitor {
  ...
  using visited_error = std::variant<error_let_with_no_bindings,
