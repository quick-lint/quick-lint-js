...
TEST_F(test_parse_expression_typescript, angle_type_assertion) {
  {
    test_parser& p = this->make_parser(u8"<Type>expr"_sv);
    EXPECT_THAT(p.errors(), IsEmpty());
    ...
    EXPECT_EQ(p.range(ast).begin_offset(), 0);
    EXPECT_EQ(p.range(ast).end_offset(), strlen(u8"<Type>expr"));
  ...
  {
    test_parser& p = this->make_parser(u8"<Type>expr;\n// </Type>;"_sv);
    EXPECT_THAT(p.errors(), IsEmpty());
    expression* ast = p.parse_expression();
...
TEST(test_parse_expression_typescript_statement, angle_type_assertion) {
  {
    parse_visit_collector v =
...
TEST(test_parse_expression_typescript_statement,
  ...
  {
    padded_string code(u8"function f(<T>x) {}"_sv);
    spy_visitor v;
    parser p(&code, &v, typescript_options);
    p.parse_and_visit_module(v);
    EXPECT_THAT(v.errors, ElementsAre(DIAG_TYPE_OFFSETS(
