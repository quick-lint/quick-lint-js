...
TEST_F(test_parse_typescript_type,
       tuple_type_spread_element_with_optional_element) {
...
TEST_F(test_parse_typescript_type, no_question_in_param_type) {
  {
    test_parser p(u8"fs.promises.writeFile(outputPath, result).then((err: Error?) => {if (err) throw err;});"_sv,
    ...
    EXPECT_THAT(
        ...
        ElementsAre(DIAG_TYPE_OFFSETS(
            ...
            question, strlen(u8"fs.promises.writeFile(outputPath, result).then((err: Error"), u8"?")));
  }

  {
    ...
    EXPECT_THAT(
        ...
        ElementsAre(DIAG_TYPE_OFFSETS(
            p.code, diag_typescript_question_in_parameters_should_be_void,
            question, strlen(u8"fs.promises.writeFile(outputPath, result).then((err: "), u8"?")));
  }
}
