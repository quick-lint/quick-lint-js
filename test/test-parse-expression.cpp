...
TEST_F(test_parse_expression, parse_broken_math_expression) {
...

  {
    ...
    EXPECT_THAT(
        p.errors(),
        ElementsAre(
            ERROR_TYPE_OFFSETS(p.code(), diag_missing_operand_for_operator,  //
                               where, strlen(u8"2 "), u8"&"),
            ERROR_TYPE_OFFSETS(p.code(), diag_missing_operand_for_operator,  //
                               where, strlen(u8"2 & "), u8"&")));
  }
...

  {
    ...
    EXPECT_THAT(
        p.errors(),
        ElementsAre(ERROR_TYPE_OFFSETS(p.code(), diag_unmatched_parenthesis,  //
                                       where, strlen(u8"2 * (3 + "), u8"("),
                    ERROR_TYPE_OFFSETS(p.code(), diag_unmatched_parenthesis,  //
                                       where, strlen(u8"2 * "), u8"(")));
  }
}
...
TEST_F(test_parse_expression, parse_invalid_assignment) {
  ...
  for (const char8* code : {
    ...
    EXPECT_THAT(
        p.errors(),
        ElementsAre(ERROR_TYPE(diag_invalid_expression_left_of_assignment)));
  }
}
...
TEST_F(test_parse_expression, untagged_template_with_invalid_escape) {
  {
    ...
    EXPECT_THAT(
        p.errors(),
        ElementsAre(ERROR_TYPE(diag_expected_hex_digits_in_unicode_escape)));
  }
...
TEST_F(test_parse_expression, malformed_object_literal) {
  ...
  for (string8 op : {u8"++", u8"--"}) {
    ...
    EXPECT_THAT(
        p.errors(),
        UnorderedElementsAre(
            ERROR_TYPE_OFFSETS(p.code(), diag_missing_key_for_object_entry,  //
                               expression, strlen(u8"{"), u8"one " + op),
            // TODO(strager): Don't report
            // diag_missing_comma_between_object_literal_entries.
            ERROR_TYPE(diag_missing_comma_between_object_literal_entries)));
  }
