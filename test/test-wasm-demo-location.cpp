...
namespace {
TEST(test_wasm_demo_location, ranges_on_first_line) {
  padded_string code(u8"let x = 2;"_sv);
  wasm_demo_locator l(&code);
  wasm_demo_source_range x_range =
      l.range(source_code_span(&code[4], &code[5]));

  EXPECT_EQ(x_range.begin, 4);
  EXPECT_EQ(x_range.end, 5);
}

TEST(test_wasm_demo_location, ranges_on_second_line) {
  for (string8_view line_terminator : line_terminators_except_ls_ps) {
    ...
    wasm_demo_locator l(&code);
    wasm_demo_source_range x_range = l.range(source_code_span(y, y + 1));
...
TEST(test_wasm_demo_location, lf_cr_is_two_line_terminators) {
  padded_string code(u8"let x = 2;\n\rlet y = 3;"_sv);
  const char8* y = strchr(code.c_str(), u8'y');
  wasm_demo_locator l(&code);
  wasm_demo_source_range y_range = l.range(source_code_span(y, y + 1));

  EXPECT_EQ(y_range.begin, y - code.c_str());
}

TEST(test_wasm_demo_location, location_after_null_byte) {
  padded_string code(string8(u8"hello\0beautiful\nworld"_sv));
  const char8* r = &code[18];
  ASSERT_EQ(*r, u8'r');

  wasm_demo_locator l(&code);
  wasm_demo_source_range r_range = l.range(source_code_span(r, r + 1));

  EXPECT_EQ(r_range.begin, r - code.c_str());
}

TEST(test_wasm_demo_location, position_backwards) {
  padded_string code(u8"ab\nc\n\nd\nefg\nh"_sv);

  std::vector<wasm_demo_source_offset> expected_positions;
  {
    wasm_demo_locator l(&code);
  ...
  std::vector<wasm_demo_source_offset> actual_positions;
  {
    wasm_demo_locator l(&code);
...
TEST(test_wasm_demo_location, position_after_multi_byte_character) {
...
TEST(test_wasm_demo_location, position_after_wide_multi_byte_character) {
