...
For documents with the following document language IDs, quick-lint-js inspects the document's URI's base name. If the base name contains ".d.", then quick-lint-js interprets the documents as if they were TypeScript definition files (like the
ifdef::backend-html5[]
link:../../cli/#language[*--language=experimental-typescript-definition* command line option]).
endif::[]
ifndef::backend-html5[]
*--language=experimental-typescript-definition* command-line option).
endif::[]
Otherwise, if the base name does not contain ".d.", then quick-lint-js interprets the documents as if they were TypeScript files (like the
ifdef::backend-html5[]
...
quick-lint-js interprets documents with the following document language IDs as if they were TypeScript definition files (like the
ifdef::backend-html5[]
link:../../cli/#language[*--language=experimental-typescript-definition* command line option]):
endif::[]
ifndef::backend-html5[]
