...
#undef _
  ...
  static_assert(initial_character_class_table[static_cast<std::uint8_t>(u8'\'')] == character_class::quote);
  static_assert(initial_character_class_table[static_cast<std::uint8_t>(u8'\0')] == character_class::error);
  static_assert(initial_character_class_table[static_cast<std::uint8_t>(u8'\\')] == character_class::ident);
  static_assert(initial_character_class_table[static_cast<std::uint8_t>(u8'\x01')] == character_class::error);
  static_assert(initial_character_class_table[static_cast<std::uint8_t>(u8'\x7f')] == character_class::error);
...
#undef QLJS_STATE
  ...
  // @
  done_invalid_character : {
    if (l->input_[0] == u8'\0') {
      if (l->is_eof(l->input_)) {
        l->last_token_.type = token_type::end_of_file;
        l->last_token_.end = l->input_;
        return true;
      }
    }
    ...
    if (l->input_[0] == u8'@') {
      const char8* end = l->input_ + 1;
      l->diag_reporter_->report(diag_unexpected_at_character{
          .character = source_code_span(l->input_, end)});
      l->input_ = end;
      l->skip_whitespace();
      return false;
    } else {
      const char8* end = l->input_ + 1;
      l->diag_reporter_->report(diag_unexpected_control_character{
          .character = source_code_span(l->input_, end)});
      l->input_ = end;
      l->skip_whitespace();
      return false;
