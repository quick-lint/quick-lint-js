...
class result {
 public:
  ...
  template <class Func>
  auto visit_error(Func& visitor) const {
    QLJS_ASSERT(!this->ok());
    // TODO(strager): If std::is_same_v<Error, T>, then std::visit is incorrect.
    return std::visit(error_visitor<Func&>{visitor}, this->data_);
  }

  template <class Func>
  auto visit_error(const Func& visitor) const {
    QLJS_ASSERT(!this->ok());
    // TODO(strager): If std::is_same_v<Error, T>, then std::visit is incorrect.
    return std::visit(error_visitor<const Func&>{visitor}, this->data_);
  }
 ...
 private:
  ...
  struct error_visitor {
    ...
    auto operator()(const T&) const noexcept { QLJS_UNREACHABLE(); }

    template <class Error>
    auto operator()(const Error& error) const noexcept {
      return std::forward<Func>(this->user_visitor)(error);
    }
  };
